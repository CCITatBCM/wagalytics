...
def token(request, site_id=None):
    ...
    if hasattr(settings, "WAGALYTICS_SETTINGS"):
        ...
        if wagalytics_settings.get("GA_KEY_CONTENT", None):
            access_token = get_access_token_from_str(
                wagalytics_settings["GA_KEY_CONTENT"]
            )
...
def dashboard(request, site_id=None):
    ...
    if not site_id and (
        hasattr(settings, "WAGALYTICS_SETTINGS")
        and len(settings.WAGALYTICS_SETTINGS) > 0
    ):
        # This site has multi-site wagalytics enabled.
        # Redirect user to ^analytics/dashboard/{site_id}/$
        return redirect(
            "wagalytics_site_dashboard", site_id=Site.find_for_request(request).id
        )
    ...
    if site_id:
        ...
        if "wagtail.contrib.settings" not in settings.INSTALLED_APPS:
            display_message = (
                "You must enable the Wagtail Site Settings app for "
        ...
        if not hasattr(settings, "WAGALYTICS_SETTINGS"):
            ...
            display_message = (
                "You have Wagalytics Multisite settings, but not "
        ...
        try:
            ...
            display_message = (
                "You are missing your GA_VIEW_ID setting. Your "
                "analytics dashboard won't load without this setting."
            )
            messages.error(request, display_message)

        if not hasattr(settings, "GA_KEY_FILEPATH") and not hasattr(
            settings, "GA_KEY_CONTENT"
        ):
            display_message = (
                "You are missing your GA_KEY_FILEPATH or your "
